  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: resultData } = await api.get(`/results/${examId}`);
        setExam(examData);
  ...
  return (
    <div className={styles.examContainer}>
      ...
      <div className={styles.scoreSummary}>
        <h2>Your Score: {results.score}/{exam.questions.length}</h2>
        <p>Submission Time: {new Date(results.submittedAt).toLocaleString()}</p>
      </div>
      ...
      <div className={styles.resultsDetails}>
        {exam.questions.map((question, index) => {
          ...
          return (
            <div 
              key={question.id}
              className={`${styles.questionResult} ${isCorrect ? styles.correct : styles.incorrect}`}
            >
                ...
                <>
                  ...
                  <div className={styles.options}>
                    {question.options.map(opt => (
                      <span 
                        key={opt}
                        className={`${styles.option} ${opt === question.correctAnswer ? styles.correctOption : ''}`}
                      >
                        {opt}
                      </span>
                    ))}
